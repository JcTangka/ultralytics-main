...
def split_yolo_dataset(images_dir, labels_dir, output_dir, train_ratio=0.8, random_seed=42):
    """参数说明： - images_dir: 原始图片目录路径 - labels_dir: 原始标签目录路径 - output_dir: 输出根目录路径 - train_ratio: 训练集比例（默认0.8） -
    ...
    # 获取有效文件对
    valid_pairs = [
        (f, os.path.splitext(f)[0] + ".txt") for f in image_files if os.path.splitext(f)[0] in label_basenames
    ]
